 ...
 * General Interface Test Automation Kit (GITAK) 0.8.1, 0.9.1
 */
...
objectExtend(HtmlTestRunner.prototype, {
    ...
    startTest: function () {
        ...
        this.testFailed = false;
        //storedVars = new Object(); // do not reset, keep storeVars global.
        // create GI specific HTMLTestLoop
...
//
    ...
    post: function () {
		...
		if (Selenium.version)
			...
			form.createHiddenField("selenium.version", Selenium.coreVersion); // used in selenium-rc
			
		if (Selenium.revision)
			form.createHiddenField("selenium.revision", Selenium.revision);
		else
			form.createHiddenField("selenium.revision", Selenium.coreRevision); // used in selenium-rc
    ...
    _saveToFile: function (fileName, form) {
        ...
        // TODO -- fail tests rows added. Need to add passed tests also -- gitak
