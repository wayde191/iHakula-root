...
Sa=(x.createNode(jsx3.xml.Entity.TYPEELEMENT,ub.x)).setValue(X(hb,ub.A));S.appendChild(E);S.appendChild(ia);S.appendChild(Sa);ab.appendChild(S);}return x;};r.save=function(){if(this._error){return false;}else{var
M=this._toXML();if(jsx3.ide.writeUserXmlFile(this.getOpenFile(),jsx3.ide.makeXmlPretty(M,true))){this.setDirty(false);this.publish({subject:ub.B});return true;}else return false;}};r.getPlugIn=function(){return jsx3.IDE.GitakEditorPlugin;};r.onEditRecord=function(o,k){this.setDirty(true);};r.onLaunchPlayer=function(c){var
...
B=this.jsxsuper(h);return B?ub.J+B:B;};r.kL=function(d,s){var
