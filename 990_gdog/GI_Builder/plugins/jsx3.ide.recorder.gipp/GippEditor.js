...
O=wa.substring(sb+q.START.length,Sa);this.zU(O);this._prefix=wa.substring(0,sb);this._suffix=wa.substring(Sa+q.END.length);}else{((this.getPlugIn()).getServer()).alert(null,ub.e);this._error=true;}}else{this._prefix=ub.f;this._suffix=ub.g;}};r.save=function(){if(this._error){return false;}else{var
xb=this._prefix+q.START+ub.h+this.tr()+ub.h+q.END+this._suffix;if(jsx3.ide.writeUserFile(this.getOpenFile(),xb)){this.setDirty(false);this.publish({subject:ub.i});return true;}else return false;}};r.tr=function(){var
xa=ub.j;for(var
Hb=((this.Bk()).getXML()).getChildIterator();Hb.hasNext();){var
db=Hb.next();var
F=jsx3.$O.json(db.getAttribute(ub.k));var
U=jsx3.$O.json(db.getAttribute(ub.l));var
ga=jsx3.$O.json(db.getAttribute(ub.m));var
_=jsx3.$O.json(db.getAttribute(ub.n));xa=xa+(ub.o+F+ub.p+U+ub.q+ga+ub.r+_+ub.s);if(Hb.hasNext())xa=xa+ub.t;}xa=xa+ub.u;return xa;};r.zU=function(d){try{var
...
V=this.Bk();var
w=(this.Bk()).getRecord(o);if(this.Ay(w)){V.insertRecordProperty(o,ub.y,q._DIVCLASS,true);}else V.insertRecordProperty(o,ub.y,ub.z,true);}this.setDirty(true);};r.onLaunchPlayer=function(l){var
