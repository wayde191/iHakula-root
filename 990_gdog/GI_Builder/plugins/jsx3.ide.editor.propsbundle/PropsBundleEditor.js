...
kb=(jsx3.IDE.PropsBundleEditorPlugin.getResource(ub.a)).getData();return f.loadXML(kb,false,this.getPlugIn());};c.Dc=function(){return (this.getContent()).getDescendantOfName(ub.b);};c.nm=function(n){var
U=this.Dc();U.setSourceXML(this.K3(n));var
qb=U.getChild(ub.c);for(var
Wa=(U.getChildren()).length-1;Wa>qb.getChildIndex();Wa--)U.removeChild(Wa);var
Ab=this.vx(n);Ab.sort();for(var
Wa=0;Wa<Ab.length;Wa++)this._addColumn(Ab[Wa],U,qb);};c._addColumn=function(f,n,p){if(n==null)n=this.Dc();if(p==null)p=n.getChild(ub.c);var
Nb=jsx3.util.Locale.valueOf(f);var
Db=p.doClone();Db.setName(Nb.toString());Db.setText(Nb.getDisplayName()||Nb.toString());Db.setPath(Nb.toString());Db.setTip(Nb.toString()+ub.d+Nb.getDisplayName());Db.setFormatHandler(i.FORMATTER);};c.Me=function(){return this.EE((this.Dc()).getXML());};c.K3=function(n){var
...
T=Qa.getAttribute(ub.i);if(wb[cb]==null)wb[cb]=N.insertRecord({jsxid:cb});wb[cb].setAttribute(ra,T);}}return N;};c.EE=function(k){var
Hb=(new
...
x=Hb.createNode(jsx3.xml.Entity.TYPEELEMENT,ub.l);x.setAttribute(ub.h,O);x.setAttribute(ub.i,sa.getAttribute(ab));z[ab].appendChild(x);}}return Hb;};c.vx=function(b){var
eb=[];for(var
...
T=this;var
rb=this.Dc();((this.getPlugIn()).getServer()).prompt(ub.m,ub.n,function(e,l){e.doClose();l=jsx3.util.strTrim(l);if(rb.getChild(l)){((this.getPlugIn()).getLog()).error(ub.o+l+ub.p);}else if(l.length>0){T._addColumn(l);T.setDirty(true);}});};c.removeLocaleColumn=function(a){var
Ga=a.getParent();var
Mb=Ga.getXML();var
L=a.getPath();for(var
