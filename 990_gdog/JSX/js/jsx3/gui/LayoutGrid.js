...
_a=this.getRendered();if(_a!=null)this.Ul({},_a);};j.uK=function(m,r){var
Ab=m?ub.g:ub.h;var
N=this.Wl(true);if(N[Ab] instanceof Array)return N[Ab];if(r==null||isNaN(r))r=m?N.Zg():N.Sh();if(isNaN(r))return [];var
R=0;var
_a=0;var
...
T=0;for(var
x=0;x<y.length;x++){T=T+y[x]%1;y[x]=Math.floor(y[x]);if(T>=1||x==y.length-1&&T>0.5){y[x]++;T--;}}N[Ab]=y;return y;};j.ng=function(m,s){if(s==null)s=m.getChildIndex();var
...
,{boxtype:ub.k,left:u,top:Ca,width:ub.l,height:ub.l,parentwidth:qa[Cb],parentheight:sa[oa]});u=u+qa[Cb];}Ca=Ca+sa[oa];}};j.gi=function(s,p,r){if(!p){this.clearBoxProfile(true);return;}if(this.getParent()&&(!isNaN(s.parentwidth)&&!isNaN(s.parentheight)||s.width==null&&s.height==null)){s=(this.getParent()).ng(this);}else if(s==null)s={};var
...
sa=0;sa<D.length;sa++)r.add(D[sa],this.ng(D[sa],sa),p?p.childNodes[sa]:null,true);};j.V8=function(q){if(!q)q=this.Wl(false);if(q){delete q[ub.g];delete q[ub.h];}};j.Jm=function(o){if(this.getParent()&&(o==null||!isNaN(o.parentwidth)&&!isNaN(o.parentheight)||!isNaN(o.width)&&!isNaN(o.height))){o=(this.getParent()).ng(this);}else if(o==null)o={};var
...
Ra=0;Ra<ba.length;Ra++)ba[Ra].tl(this.ng(ba[Ra],Ra));return this.jsxsuper();};g.getVersion=function(){return ub.q;};});jsx3.LayoutGrid=jsx3.gui.LayoutGrid;
