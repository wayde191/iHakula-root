...
Q=jsx3.gui.Interactive;m.ORIENTATIONH=0;m.ORIENTATIONV=1;m.Jo=ub.a;m.SK=ub.b;jsx3.html.loadImages(m.Jo,m.SK);m.n9=ub.c;m.rv=ub.d;m.t3=ub.e;f.init=function(i,s){this.jsxsuper(i);if(s!=null)this.setOrientation(s);for(var
X=1;X<=2;X++){var
wa=new
...
Db,db,oa,rb;if(La){Db=Ra;db=0;oa=y;rb=D;}else{Db=0;db=Ra;oa=Ja;rb=y;}return this.Ck(bb,{boxtype:ub.k,tagname:ub.l,left:Db,top:db,width:oa,height:rb,parentwidth:oa,parentheight:rb});};f.gi=function(c,s,o){var
...
ka=ea.lg(0);if(bb){ka.recalculate({left:Pa,parentheight:P},s?s.childNodes[0]:null,o);}else ka.recalculate({top:Pa,parentwidth:Mb},s?s.childNodes[0]:null,o);}o.add(L[va],la,s?s.childNodes[va+1]:null,true);Mb=la.width;P=la.height;Pa=bb?Mb:P;}}};f.Jm=function(c){this.applyDynamicProperties();if(this.getParent()&&(c==null||!isNaN(c.parentwidth)&&!isNaN(c.parentheight)||!isNaN(c.width)&&!isNaN(c.height))){c=(this.getParent()).ng(this);}else if(c==null)c={};var
...
sa=this.ep();Kb.parentwidth=cb.Sh();Kb.parentheight=cb.Zg();var
...
ja=new
jsx3.gui.Painted.Box(Kb);cb.Td(ja);return cb;};f.paint=function(){this.applyDynamicProperties();var
ma=this.pi(ub.m,ub.n,1);var
K=this.renderAttributes(null,true);var
vb=this.Wl(true);vb.setAttributes(this.md()+this.Do()+ub.o+this.getId()+ub.p+this.Wb()+K);vb.setStyles(this.Uc()+this.qg()+this.Ne()+this.Vg());var
...
jb,fa;if((jb=this.getChild(0))!=null)jb.tl(this.ng(jb));if((fa=this.getChild(1))!=null)fa.tl(this.ng(fa));return (vb.paint()).join((rb.paint()).join(ub.v)+this.paintChildren());};f.Uc=function(){return ub.w+(this.getBackgroundColor()?this.getBackgroundColor():m.rv)+ub.x;};f.doBeginMove=function(e,h){if(!e.leftButton()||!this._isEnabled())return;if(this.doEvent(ub.y,{objEVENT:e,objGUI:h})===false)return;h.style.backgroundColor=m.n9;this.focus();this._jsxmoving=true;if(this.getOrientation()==0){jsx3.EventHelp.constrainY=true;}else jsx3.EventHelp.constrainX=true;this.jsxsupermix(e,h);W.subscribe(ub.z,this,ub.A);};f._doResizeEnd=function(p){p=p.event;var
