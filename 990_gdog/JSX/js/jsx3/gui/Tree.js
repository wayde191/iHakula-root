...
Za=e[wa];if(Za!=null&&!this.kk(Za))continue;this.k4(Za);}if(a&&e.length>0)this.revealRecord(e[0]);if(Nb)this.doEvent(ub.w,{strRECORDID:e[0],strRECORDIDS:e});}else{if(e!=null&&!this.kk(e))return this;this.gw();if(e!=null){this.k4(e);if(a)this.revealRecord(e);}if(Nb)this.doEvent(ub.w,{strRECORDID:e,strRECORDIDS:[e]});}if(Nb)this.doEvent(ub.x,{objEVENT:null,preVALUE:ba,newVALUE:this.getValue(),_gipp:1});return this;};b.A4=function(h,r,i,p,f){var
u=this.getValue();var
Va=this.getMultiSelect()==1;if(Va&&i){if(r!=null&&!this.kk(r))return;var
Z=false;if(this.Zs(r)){if(p)this.aQ(r);}else{this.k4(r);Z=true;}if(Z&&!f)this.doEvent(ub.w,{objEVENT:h,strRECORDID:r,strRECORDIDS:[r]});}else{var
xa=this.getValue()==r;if(!p&&xa&&!i)return;this.gw();if(r!=null&&!this.kk(r))r=null;if(r!=null)if(xa)this.aQ(r);else this.k4(r);if(!f){var
eb=xa?null:r;var
...
na=B.selectSingleElm(Nb,0,2);jsx3.html.addClass(na,ub.B);na.style.backgroundImage=ub.j+j.SELECTEDIMAGE+ub.l;}};b.bL=function(a){var
...
Ua=Q.next();Ua.removeAttribute(ub.p);this.bL(Ua.getAttribute(ub.z));}};b.aQ=function(r){this.deleteRecordProperty(r,ub.p,false);this.bL(r);};b.k4=function(o){this.insertRecordProperty(o,ub.p,ub.q,false);this.UA(o);};b.Ww=function(){return (this.getXML()).selectNodeIterator(ub.C);};b.c8=function(){var
Nb=this.Ww();var
...
H=this.getDocument();return H!=null?H.getElementById(this.getId()+ub.F+g):null;};b.Vl=function(e,h){if(h!=e.srcElement())return;var
Ma=(this.c8())[0];if(Ma){this.zQ(Ma);}else{var
sa=(this.getRendered(h)).childNodes[0];if(sa!=null)this.zQ(sa);}};b.Pb=function(o,k){if(!o.leftButton())return;k=o.srcElement();var
Db=this.getRendered(k);while(jsx3.util.strEmpty(k.getAttribute(ub.G))&&k!=Db)k=k.parentNode;if(k.getAttribute(ub.G)!=null){if(k.getAttribute(ub.G)==ub.H){this.EG(o,k.parentNode.parentNode.getAttribute(ub.z));}else if(k.getAttribute(ub.G)==ub.I||k.getAttribute(ub.G)==ub.J){var
Ib=k.parentNode.parentNode.getAttribute(ub.z);var
Na=this.yH();this.zQ(k.parentNode.parentNode);if(!k.parentNode)k=(this.BB(Ib)).childNodes[0].childNodes[2];if(this.kk(Ib))if(o.shiftKey()&&this.getMultiSelect()==1){if(Na){this.revealRecord(Na);this.ap(o,Na,Ib);}else this.A4(o,Ib,false,true);}else{var
...
Ga=this.c8();if(Ga.length==1)return Ga[0];return null;};b.ap=function(k,c,h){var
...
O=0;O<N.length;O++){var
K=N[O];var
fb=this.Zs(K);if(ua||fb)this.A4(null,K,true,!ua&&fb,true);}if(ua)this.doEvent(ub.w,{objEVENT:k,strRECORDID:Fa[0],strRECORDIDS:Fa});this.doEvent(ub.x,{objEVENT:k,preVALUE:ob,newVALUE:this.getValue(),_gipp:1});};b.kk=function(q){var
...
sa=this.BB(i);if(sa!=null)this.dP(vb,sa,e);return this;};b.EG=function(n,l,r,s){var
...
C=jsx3.gui.isMouseEventModKey(n);Ta=this.dP(Ab,_,r,C);}this.doEvent(ub.R,{objEVENT:n,strRECORDID:l,objRECORD:Ab,bOPEN:Ta,_gipp:1});}};b.dP=function(p,s,a,e){var
...
za=this.BB(_a.getAttribute(ub.z));if(za)this.dP(_a,za,a,true);}}return a;};b.Xg=function(r,n){if(this.getCanDrag()==1&&!r.rightButton()){var
Db=r.srcElement();if(Db==null)return;var
Kb=false;if(jsx3.util.strEmpty(Db.getAttribute(ub.G)))Db=Db.parentNode;if(Db.getAttribute(ub.G)==ub.J){Kb=true;Db=Db.parentNode.childNodes[2];}if(Db.getAttribute(ub.G)==ub.I){if(!this.kk(Db.getAttribute(ub.V)))return;var
Aa=this;r.rj();j.B2=window.setTimeout(function(){j.B2=null;lb.unsubscribe(ub.W,Aa,ub.X);if(Aa.getParent()!=null){Aa.Pb(r,n);Aa.doDrag(r,Db,j.getDragIcon,{strDRAGIDS:Aa.c8()});}},j.Yw);lb.subscribe(ub.W,this,ub.X);if(Kb){lb.publish(r);r.cancelAll();}}}};b.QA=function(m){lb.unsubscribe(ub.W,this,ub.X);if(j.B2)window.clearTimeout(j.B2);};b.eo=function(n,r){var
...
cb=(this.getServer()).getJSX(this.getMenu());if(cb!=null&&this.kk(xa)){var
...
Xa=i.clientY()+jsx3.EventHelp.DEFAULTSPYTOPOFFSET;i.rj();var
...
wa=this;q.rj();j.aZ=window.setTimeout(function(){delete j[ub.fa];if(wa.getParent()!=null)wa.EG(q,Hb);},j.aR);}}else if(_==ub.I){var
...
x=false;if(xb==37){if(x&&V){this.EG(f,rb,false);}else this.x3(w);}else if(xb==38){this.x3(w);}else if(xb==39){if(na&&!V){var
Nb=this;this.EG(f,rb,true,function(){Nb.zQ(rb);});}else if(x&&!V){this.EG(f,rb,true);}else this.rJ(w);}else if(xb==40)this.rJ(w);f.cancelAll();}else if(xb==9){if(f.shiftKey()){B.focusPrevious(this.getRendered(s),f);}else B.focusNext(this.getRendered(s),f);}else if(f.spaceKey()||f.enterKey()){var
...
na=Ua[Ya];if(na==null||!this.kk(na))continue;var
G=this.getRecordNode(na);var
pa=G.getAttribute(ub.na);if(pa){var
tb={strRECORDID:na};tb.objRECORD=G;if(k instanceof jsx3.gui.Event)tb.objEVENT=k;this.eval(pa,this._getEvtContext(tb));}}if(k)this.doEvent(ub.na,{objEVENT:k,objRECORD:this.getRecordNode(Ua[0]),strRECORDIDS:Ua,strRECORDID:Ua[0]});};b.Zn=function(l,a){var
ta=null;var
ob=l.srcElement();if(ob!=null&&jsx3.util.strEmpty(ob.getAttribute(ub.G)))ob=ob.parentNode;if(ob!=null&&ob.getAttribute(ub.G)!=null&&(ob.getAttribute(ub.G)==ub.I||ob.getAttribute(ub.G)==ub.J))ta=ob.parentNode.parentNode.getAttribute(ub.z);if(ta)this.n2(l);};j.ld={};j.ld[ub.oa]=true;j.ld[ub.pa]=true;j.ld[ub.qa]=true;j.ld[ub.ra]=true;j.ld[ub.W]=true;j.ld[ub.sa]=true;j.ld[ub.ta]=true;j.ld[ub.ua]=true;j.ld[ub.va]=true;b.gi=function(f,c,l){this.Tl(f,c,l,3);};b.Jm=function(r){this.applyDynamicProperties();if(this.getParent()&&(r==null||isNaN(r.parentwidth)||isNaN(r.parentheight))){r=(this.getParent()).ng(this);}else if(r==null)r={};r.boxtype=ub.wa;r.tagname=ub.xa;if(r.left==null)r.left=0;if(r.top==null)r.top=0;if(r.width==null)r.width=ub.ya;if(r.height==null)r.height=ub.ya;var
...
Ca=this.getId();var
A=this.doTransform();if(!A)A=this.getNoDataMessage();A=A+(ub.za+jsx3.gui.Block.SPACE+ub.Aa+this.pi(ub.va,ub.Ba)+ub.Ca+this.md()+ub.Da);var
F=ub.r;if(this.getEnabled()==1)F=this.Bf(j.ld,0);var
Xa=this.renderAttributes(null,true);var
Fa=this.Wl(true);Fa.setAttributes(this.Do()+F+ub.Ea+Ca+ub.Aa+this.Wb()+ub.Fa+Xa);Fa.setStyles(this.Di()+this.Uc()+this.ti()+this.Kh()+this.Bj()+this.Gd()+this.We()+this.yg()+this.qg()+this.Ne()+this.Vg());return (Fa.paint()).join(A+ub.Ga);};b.doTransform=function(d){var
G={};var
P=d!=null;if(!P){var
