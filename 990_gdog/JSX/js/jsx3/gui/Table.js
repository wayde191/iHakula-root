...
cb=jsx3.xml.CDF;h.DEFAULTXSLURL=jsx3.resolveURI(ub.a);h.Sx=(new
...
Lb=l.scrollLeft;l.parentNode.childNodes[1].childNodes[0].style.left=ub.o+Lb+ub.p;}};a.getSelectionModel=function(p){return this.jsxselectionmodel!=null?this.jsxselectionmodel>2?0:this.jsxselectionmodel:p!=null?p:null;};a.setSelectionModel=function(g){this.jsxselectionmodel=g;};a.Pb=function(l,n){var
...
ma=Ab.getAttribute(ub.r);if(jsx3.gui.isMouseEventModKey(l)){this.fS(ma);if(this.isRecordSelected(ma)){this.sZ(l,ma,null);}else this.GI(l,ma,null,true);}else if(l.shiftKey()){var
t=this.uD();if(t){var
bb=this.wN(t);if(bb)this.GS(l,bb,Ab);}else{this.fS(ma);this.GI(l,ma,null,false);}}else{this.fS(ma);if(!this.isRecordSelected(ma))this.GI(l,ma,null,false);}this.focus();};a.dX=function(){if(!this._jsxUY)this._jsxUY={bg:(this.getServer()).resolveURI(this.getSelectionBG(h.SELECTION_BG))};return this._jsxUY.bg;};a.getSelectionBG=function(q){return this.jsxselectionbg?this.jsxselectionbg:q?q:null;};a.setSelectionBG=function(d){delete this[ub.s];this.jsxselectionbg=d;};a.fS=function(j){this._jsxkT=j;};a.uD=function(){return this._jsxkT;};a.gD=function(){return (this.getDocument()).getElementById(this.uD());};a.getSelectedNodes=function(){return (this.getXML()).selectNodes(ub.t+ub.u+ub.v);};a.getSelectedIds=function(){var
bb=[];var
na=(this.getXML()).selectNodeIterator(ub.t+ub.u+ub.v);while(na.hasNext()){var
Q=na.next();bb[bb.length]=Q.getAttribute(ub.r);}return bb;};a.kk=function(f){var
C=this.getRecord(f);return C&&(C[ub.w]==null||C[ub.w]!=ub.x);};a.isRecordSelected=function(r){var
X=this.getRecord(r);return X!=null&&X[ub.u]==ub.x;};a.selectRecord=function(b){if(this.getSelectionModel()==0)return;if(!this.kk(b))return;this.GI(false,b,null,this.getSelectionModel()==2);};a.deselectRecord=function(q){this.sZ(false,q,null);};a.deselectAllRecords=function(){var
Ab=this.getSelectedIds();var
Ha=Ab.length;for(var
_a=0;_a<Ha;_a++)this.sZ(false,Ab[_a]);};a.GI=function(m,r,q,i){var
...
X=0;X<q.childNodes.length;X++)q.childNodes[X].style.backgroundImage=lb;}this.Wz(m,r);return true;};a.sZ=function(r,j,f){var
...
da=0;da<f.childNodes.length;da++)f.childNodes[da].style.backgroundImage=ub.A;}this.Wz(r);return true;};a.GS=function(q,m,n){if(!m||!n)return;var
lb=m.getAttribute(ub.r);var
zb=n.getAttribute(ub.r);if(!this.kk(lb)||!this.kk(zb))return;var
...
I={};for(var
z=0;z<ja;z++){var
C=this.wN(Wa[z]);if(C.getAttribute(ub.B)<ba||C.getAttribute(ub.B)>W){this.sZ(false,Wa[z],C);}else I[Wa[z]]=1;}var
J=n.parentNode;for(var
z=ba;z<=W;z++){var
T=J.childNodes[z].getAttribute(ub.r);if(!I[T])this.GI(false,T,J.childNodes[z],true);}this.Wz(q,zb);};a.Wz=function(f,j){if(!this._jsxJX)this._jsxJX=[];if(f&&f instanceof Mb){var
...
ab=(this.getServer()).getJSX(this.getMenu());if(ab!=null&&this.kk(X)){var
...
_=m.clientY()+jsx3.EventHelp.DEFAULTSPYTOPOFFSET;m.rj();var
...
va=db[T];if(va==null||!this.kk(va))continue;var
z=this.getRecordNode(va);var
Q=z.getAttribute(ub.O);if(Q){var
Wa={strRECORDID:va};if(b instanceof Mb)Wa.objEVENT=b;this.eval(Q,this._getEvtContext(Wa));}}if(b)this.doEvent(ub.O,{objEVENT:b,strRECORDIDS:this.getSelectedIds(),strRECORDID:db[0]});};a.Zn=function(o,q){var
...
la=P.getAttribute(ub.r);if(la)this.n2(o,la);};h.ld={};h.ld[ub.P]=true;h.ld[ub.Q]=true;h.ld[ub.R]=true;h.ld[ub.S]=true;h.ld[ub.T]=true;h.ld[ub.U]=true;a.gi=function(m,j,e){this.Tl(m,j,e,3);if(j)jsx3.sleep(function(){this.es(this.getRendered());},this.getId()+ub.V,this);};a.Jm=function(m){this.applyDynamicProperties();if(this.getParent()&&(m==null||isNaN(m.parentwidth)||isNaN(m.parentheight))){m=(this.getParent()).ng(this);}else if(m==null)m={};m.boxtype=ub.W;m.tagname=ub.X;if(m.left==null)m.left=0;if(m.top==null)m.top=0;if(m.width==null)m.width=ub.Y;if(m.height==null)m.height=ub.Y;var
...
Sa=ub.A;if(this.getEnabled()==1)Sa=this.Bf(h.ld,0);var
Xa=this.renderAttributes(null,true);var
ab=this.Wl(true);ab.setAttributes(this.Do()+Sa+ub.Z+Z+ub._+this.Wb()+ub.aa+Xa);ab.setStyles(this.Di()+this.Uc()+this.Kh()+this.Gd()+this.We()+this.yg()+this.qg()+this.Ne()+this.Vg());return (ab.paint()).join(ub.ba+this.pi(ub.ca,ub.da)+ub.ea+za+ub.fa);};a.onAfterPaint=function(n){this.eB(n);};a.onAfterRestoreView=function(l){this.es(l);};a.eB=function(b){var
ha=this.getId();var
