...
Db=(this.getChildren()).length==1;da=this.Xr(da,Db);jsx3.html.insertAdjacentHTML(Na,ub.c,c.paintChildren([da]));this.jsxsuper(da,l,Na,true);if(Db)this.wk(null,c);}this.Zf();};k.onSetChild=function(g){if(!(g instanceof V))return false;var
Va=g.getContentChild();if(Va&&Va instanceof Bb)Va.setVisibility(ub.d);if((this.getChildren()).length==0)this.jsxselectedindex=-1;return true;};k.Xr=function(q,m){q=q.Dk(m)||q;if(q instanceof Bb)q.setDisplay(m?ub.d:ub.e,true);return q;};k.onRemoveChild=function(g,m){this.jsxsuper(g,m);if(jsx3.$A.is(g)){var
...
ob={left:0,top:Z,width:_a,height:sa,parentwidth:_a,parentheight:sa,boxtype:ub.h,tagname:ub.i};if(this.getShowTabs())ob.border=ub.j;return ob;}return {};};k.gi=function(q,n,a){var
pa=this.Wl(true,q);if(n){pa.recalculate(q,n,a);var
cb=pa.lg(0);cb.recalculate({parentwidth:pa.Sh(),height:this.paintTabSize()+1},n!=null?n.childNodes[0]:null,a);var
H=cb.lg(0);H.recalculate({parentwidth:pa.Sh(),height:this.paintTabSize()+1},n!=null?n.childNodes[0].childNodes[0]:null,a);var
ia=cb.lg(2);ia.recalculate({left:cb.Sh()-22},n!=null?n.childNodes[0].childNodes[2]:null,a);var
y=this.getChildren();var
kb=this.ng();for(var
Ca=0;Ca<y.length;Ca++){var
J=y[Ca];a.add(J,this.ng(),n!=null,true);var
z=J.getContentChild();if(z!=null){var
Sa=this.getSelectedIndex()==Ca;z=this.Xr(z,Sa);if(Sa)a.add(z,this.ve(),z.getRendered(n),true);}}this.Zf();}};k.Jm=function(m){if(this.getParent()&&(m==null||!isNaN(m.parentwidth)&&!isNaN(m.parentheight)||!isNaN(m.width)&&!isNaN(m.height))){m=(this.getParent()).ng(this);}else if(m==null)m={};if(m.left==null)m.left=0;if(m.top==null)m.top=0;if(m.width==null)m.width=ub.k;if(m.height==null)m.height=ub.k;if(m.tagname==null)m.tagname=ub.i;if(!m.boxtype)m.boxtype=ub.h;var
...
v=new
jsx3.gui.Painted.Box(U);Jb.Td(v);U={};U.parentwidth=m.parentwidth;U.width=ub.k;U.height=this.paintTabSize()+1;U.left=0;U.top=0;U.tagname=ub.i;U.boxtype=ub.h;var
wb=new
jsx3.gui.Painted.Box(U);wb.setStyles(ub.l);v.Td(wb);U={};U.width=14;U.height=14;U.left=2;U.top=2;U.tagname=ub.i;U.boxtype=ub.h;var
Ha=new
jsx3.gui.Painted.Box(U);v.Td(Ha);U={};U.width=14;U.height=14;U.left=v.Sh()-16;U.top=2;U.tagname=ub.i;U.boxtype=ub.h;var
Ja=new
jsx3.gui.Painted.Box(U);v.Td(Ja);return Jb;};k.paint=function(){this.applyDynamicProperties();var
u=this.getId();var
...
v=this.Wl(true);v.setAttributes(ub.p+u+ub.q+this.Wb()+this.md()+this.Do()+A+Na);v.setStyles(this.Kk()+this.Uc()+this.ti()+this.qg()+this.Ne()+this.Vg());var
xb=v.lg(0);if(Fa){var
...
mb=Aa[ga].getContentChild();if(mb!=null){mb=this.Xr(mb,Ba);mb.tl(this.ve());fb.push(mb);}}jsx3.sleep(function(){this.Zf();},null,this);var
Ra=xb.lg(0);var
...
la=this.vn();if(la)if(Ib<qb){la.nextSibling.style.display=parseInt(la.style.left)<0?ub.d:ub.e;la.nextSibling.nextSibling.style.display=ub.d;}else{la.nextSibling.style.display=ub.e;la.nextSibling.nextSibling.style.display=ub.e;la.style.left=ub.B;}};k.wo=function(){var
...
P=wb==Xa;if(H)H=this.Xr(H,P);if(Aa)Ta.setBackgroundColor(P?Ta.kh():Ta.Al(),true);if(P)H.Ul(this.ve(),true);}}if(l)m.doEvent(ub.H,{objEVENT:l instanceof pb?l:null});if(!n){var
