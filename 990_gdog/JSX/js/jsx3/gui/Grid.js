...
ib=f.parentNode.id.substring((this.getId()).length+1);if(!this.kk(ib))return;var
vb=f!=this._jsxcg||s;if(vb&&this._jsxcurmask!=null&&(s==true||s!=true&&!(this._jsxcurmask instanceof jsx3.gui.TextBox))){this.onAfterEdit(this._jsxcg,this._jsxcurmask);}else if(vb&&this._jsxcurmask!=null)this.resetMask();try{jsx3.html.focus(f);if(this._jsxcg!=null)this._jsxcg.style.backgroundImage=this._jsxcg.prevBG!=null?this._jsxcg.prevBG:ub.q;if(this.getGrowBy()>0&&f.parentNode==f.parentNode.parentNode.lastChild)this.appendRow();}catch(Kb){var
va=ob.getElementById(this._jsxrowid);if(va!=null)va.childNodes[this._jsxcellindex].style.backgroundImage=ub.q;return false;}f.prevBG=f.style.backgroundImage;f.style.backgroundImage=l.SELECTBGIMAGE;this._jsxcg=f;this._jsxcellindex=f.cellIndex;this._jsxrowid=f.parentNode.id;var
Ja=(this.Ld())[this._jsxcellindex];if(s==null){var
da=f.getAttribute(ub.r);if(!jsx3.util.strEmpty(da)&&da!=ub.s){var
la=Ja.getDescendantOfName(da)||this.getDescendantOfName(da);this.qu(qa,f,la);}else this.doEvent(ub.t,{objEVENT:qa,strRECORDID:ib,intCOLUMNINDEX:this._jsxcellindex});}};o.qu=function(c,j,r){this.resetMask();this._jsxcurmask=r;var
...
db=false;var
ta=s.keyCode();var
_=ta>36&&ta<41;if((s.enterKey()||s.tabKey())&&!s.ctrlKey()||_&&(s.ctrlKey()||s.altKey())){if(s.enterKey()&&s.shiftKey()||ta==38){s.setKeyCode(38);}else if(s.enterKey()||ta==40){s.setKeyCode(40);}else if(s.tabKey()&&s.shiftKey()&&!(!this.getTabListener()||this.getTabListener()==1)){try{this.focus();return;}catch(Kb){}}else if(s.tabKey()&&!(!this.getTabListener()||this.getTabListener()==1))try{jsx3.html.focus((this.getRendered()).lastChild);return;}catch(Kb){}this._jsxisblur=this.getActiveMask()&&this.getActiveMask() instanceof jsx3.gui.TextBox;this._ebKeyDown(s,q);db=true;}else if(s.ctrlKey()&&s.enterKey()){var
...
Bb=Wa.ownerDocument.getElementById(this._jsxrowid);if(Bb!=null)return this._jsxrowid.substring((this.getId()).length+1);}this.QE();}return null;};o.getActiveMask=function(){return this._jsxcurmask;};o.getActiveCell=function(){return this._jsxcg;};o.getActiveRow=function(){var
...
Sa=this.getSelectedId();if(d==null)d=this._jsxcg;var
...
eb=this.doEvent(ub.D,{objEVENT:fa,strATTRIBUTENAME:Jb,strRECORDID:Sa,objMASK:p,objGUI:d,intCOLUMNINDEX:this._jsxcellindex});if(eb!==false&&p!=null){this.updateCell(Jb,Sa,d,p);if(b&&p instanceof jsx3.gui.Menu)jsx3.html.focus(this.getActiveCell());}}if(p!=null&&n!==true){this.resetMask(p);delete this[ub.E];delete p[ub.F];}};o.updateCell=function(g,f,d,p){if(p==null)p=this.getActiveMask();if(d==null)d=this._jsxcg;if(p&&d){if(g==null)g=((this.Ld())[this._jsxcellindex].getPath()).replace(ub.u,ub.q);if(f==null)f=this.getSelectedId();var
K=typeof p.getMaskValue==ub.C?p.getMaskValue():p.getValue();if(K==null)K=ub.q;this.insertRecordProperty(f,g,K,true);}};o._ebKeyDown=function(a,d){if(this.jsxsupermix(a,d))return;if(this._jsxcg==null||this._jsxcg.parentNode==null)return;var
Cb=this._jsxcg;var
...
ea=Cb.parentNode==Cb.parentNode.parentNode.lastChild;if(tb==37||a.shiftKey()&&tb==9){if(tb==9)tb=37;if(y){if(ya)F=Cb;else F=Cb.parentNode.previousSibling.childNodes[V-1];}else F=Cb.previousSibling;}else if(tb==38){if(ya)F=Cb;else F=Cb.parentNode.previousSibling.childNodes[Cb.cellIndex];}else if(tb==39||tb==9){if(tb==9)tb=39;if(na){if(ea)F=Cb;else F=Cb.parentNode.nextSibling.childNodes[0];}else F=Cb.nextSibling;}else if(tb==40)if(ea)F=Cb;else F=Cb.parentNode.nextSibling.childNodes[Cb.cellIndex];this.doFocusItem(F,true);}else if(tb==13){}else if(tb==9&&a.shiftKey()){this.focus();za=true;}else if(tb==9){jsx3.html.focus((this.getRendered()).lastChild);za=true;}if(za){a.cancelBubble();a.cancelReturn();}};o.doExecute=function(k,m){this.gk(this.isOldEventProtocol(),k,m);};o.gk=function(a,f,r){if(f==null&&this._jsxrowid)f=this._jsxrowid.substring((this.getId()).length+1);if(r==null)r=this._jsxcellindex;if(f==null)return;if(!this.kk(f))return;var
A=this.getRecordNode(f);if(A!=null)this.eval(A.getAttribute(ub.G),{strRECORDID:f,intCOLUMNINDEX:r});if(a)this.doEvent(ub.G,{objEVENT:a instanceof Ua?a:null,strRECORDID:f,intCOLUMNINDEX:r});};o.Pb=function(n,r){var
la=n.srcElement();if(la.tagName==ub.H||la.tagName==ub.I)return;var
_a=this.getRendered();while(la.tagName!=ub.J&&la!=_a)la=la.parentNode;if(la.tagName==ub.J){if(la.parentNode.getAttribute(ub.K)==null)return;if(la.cellIndex==(this.Ld()).length)return;jsx3.html.focus(la);n.cancelBubble();n.cancelReturn();}};o.uo=function(){var
rb=ub.q;var
Xa=this.Ld();var
