...
Ba=null,Ra=null;if(this.minExponent!=null){Ba=this.minExponent;}else if(this.baseAtZero)Ba=0;if(this.maxExponent!=null)Ra=this.maxExponent;O=O*b.q6;da=da*b.q6;if(Ba==null)if(O==0)Ba=0;else Ba=Math.floor(Math.log(O)/Math.log(this.base));if(Ra==null)Ra=Math.ceil(Math.log(da)/Math.log(this.base));this.Rf(ub.a,Ba);this.Rf(ub.b,Ra);return true;};m.fi=function(f){var
x=this.pj(ub.a);var
...
sb=Math.pow(this.base,Fb+1);return Ta+M*(sb-Ta)/this.majorDivisions;}};m.ud=function(){var
Ja=this.pj(ub.a);var
Za=this.pj(ub.b);var
sa=[];var
Ga=0;for(var
...
Pa=Va+Ra*(db-Va)/this.majorDivisions;sa.push(this.getCoordinateFor(Pa));Ga++;}}sa.push(this.getCoordinateFor(db));Ga++;}return sa;};m.Fl=function(j,f){var
cb=[];if(f==0){return [];}else if(f==j.length){return [];}else{var
L=this.fi(f-1);var
Cb=this.fi(f);for(var
ra=1;ra<this.minorTickDivisions;ra++){var
kb=L+ra*(Cb-L)/this.minorTickDivisions;cb.push(this.getCoordinateFor(kb));}}return cb;};m.hd=function(){return false;};m.getCoordinateFor=function(p){var
Ua=this.pj(ub.a);var
sa=this.pj(ub.b);var
...
Na=Math.log(j)/Math.log(this.base);da=Math.round(this.length*(Na-Pa)/(yb-Pa));}return this.horizontal?da:this.length-da;};b.getVersion=function(){return jsx3.chart.Cd;};});
